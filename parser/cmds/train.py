...
class Train(object):
    ...
    def add_subparser(self, name, parser):
        ...
        subparser.add_argument("--input_type", type=str, choices=["conllx", "conllu"],
                               default="conllx")
        subparser.add_argument("--positive", default='', help='path to Positive Augmented Dataset')
        subparser.add_argument("--fpredicted_pos", default='', help='path to Predicted Positive Augmented Dataset')
    ...
    def __call__(self, config):
        ...
        if config.input_type == "conllu":
            ...
            # aug = UniversalDependenciesDatasetReader()
            # aug.load(config.augment)
        else:
        ...
        if config.use_predicted:
            if config.input_type == "conllu":
                ...
                # aug_predicted = UniversalDependenciesDatasetReader()
                # aug_predicted.load(config.faugment)
            else:
        ...
        if config.use_predicted:
            ...
            # augset = TextDataset(vocab.numericalize(aug, aug_predicted))
        else:
            trainset = TextDataset(vocab.numericalize(train))
            devset = TextDataset(vocab.numericalize(dev))
            testset = TextDataset(vocab.numericalize(test))
            # augset = TextDataset(vocab.numericalize(aug))

        # print(f"Trainset  : {trainset[0]}")
        # print(f"AugmentedSet : {augset[0]}")
        ...
        test_loader= batchify(dataset=testset,
                                  batch_size=config.batch_size,
                                  n_buckets=config.buckets)
        # aug_loader, _ = batchify(dataset=augset,
        #                          batch_size=config.batch_size,
        #                          n_buckets=config.buckets,
        #                          shuffle=True)
        ...
        # print(f"{'aug:':6} {len(augset):5} sentences in total, "
        #       f"{len(aug_loader):3} batches provided")
        ...
        print(f"the score of test at epoch {best_e} is {metric.uas:.2%}")
