...
class GraphBiaffineParser(nn.Module):
    ...
    def forward(self, words, tags, sign=None,graph_arc=None, graph_rel=None):
        ...
        #print(f"Shape of x inside parser.py : {len(x)}")
        #print(f"Shape of x[0] inside parser.py : {x[0].shape}")
        # print(f"Shape of x[1] inside parser.py : {x[1].shape}")
        ...
        # print(f"Shape of arc_h : {arc_h.size()}")
        arc_d = self.mlp_arc_d(x)
        rel_h = self.mlp_rel_h(x)
        # print(f"shape of rel_h : {rel_h.size()}")
...
class BiaffineParser(nn.Module):
    ...
    def forward(self, words, tags, sign=None, graph_arc=None, graph_rel=None):
        ...
        if graph_arc is None:
            if self.same_flag:
                x, s_arc, s_rel = self.parser_graph(words,tags)
            else:
                x, s_arc, s_rel = self.parser_baseline(words,tags)
        else:
            x, s_arc, s_rel = self.parser_graph(words,tags,sign,graph_arc,graph_rel)
